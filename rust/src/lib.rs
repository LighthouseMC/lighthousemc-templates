...
pub fn _vx_on_player_join(session_id : u64) {
    ...
    let mut plot_name = (0u32, 0u32,);
    unsafe { _vx_plot_name_fmt(&mut plot_name as *mut _ as u32) };
    
    let plot_name = unsafe { String::from_raw_parts(plot_name.0 as *mut u8, plot_name.1 as usize, plot_name.1 as usize) };
    ...
    let msg = format!("Plot Name: \"{}\"", plot_name.replace("\\", "\\\\").replace("<", "\\<").replace("\"", "\\\\\""));
    unsafe { _vx_player_send_message(session_id, msg.as_ptr() as u32, msg.as_bytes().len() as u32); }
